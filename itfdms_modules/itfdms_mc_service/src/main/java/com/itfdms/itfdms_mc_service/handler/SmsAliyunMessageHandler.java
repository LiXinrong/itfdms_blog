...
import com.itfdms.itfdms_mc_service.config.SmsAliyunPropertiesConfig;
 ...
 * @Description: 阿里大鱼短息服务处理
 * @Author: lxr
 * @CreateDate: 2018-08-28 20:28
 * @UpdateUser: lxr
 * @UpdateDate: 2018-08-28 20:28
...
public class SmsAliyunMessageHandler extends AbstractMessageHandler {
    ...
    public void check(MobileMsgTemplate mobileMsgTemplate) {
        Assert.isBlak(mobileMsgTemplate.getMobile(), "手机号不能为空");
        Assert.isBlak(mobileMsgTemplate.getContext(), "短信内容不能为空");
    }
    ...
    public boolean process(MobileMsgTemplate mobileMsgTemplate) {
        ...
        //可选:模板中的变量替换JSON串,如模板内容为"亲爱的${name},您的验证码为${code}"request.setTemplateParam(mobileMsgTemplate.getContext());
